...
var Formatter = function( sink, yaml_stream )
{
    this.sink = sink;
...
Formatter.prototype.begin_array = function()
{
    ...
    this.push_format(
        format.indent + 1,
        prefix,
        prefix + YAML.array_extra
    );
};

Formatter.prototype.end_array = function()
{
    this.pop_format();
};
 ...
 * @return {*}
 */
Formatter.prototype.top_format = function()
{
...
var YAML_stream = function( sink )
{
    ...
    this.formatter = new Formatter( sink, this );
};
...
YAML_stream.prototype._machine = function()
{
    ...
    while ( this._has_token() || this._has_input() )
    {
        ...
        if ( need_lookahead )
        {
            ...
            {
                // Defense
                throw new Error( "No code to unconsume a current token back to a lookahead token." );
            }
        }
        else if ( need_token )
        {
        ...
        if ( !have_token )
        {
            ...
            let format = this.formatter.top_format();
            switch ( c_token.view.type.id )
            {
                ...
                case Encoding.type.array_stream.id:
                    ...
                    if ( is_mark( la_token ) )
                    {
                        switch ( la_token.view.type.id )
                        {
                            case Encoding.type.start.id:
                                let first_pass = !( 'state' in c_token );
                                if ( first_pass )
                                {
                                    /*
                    ...
                    else
                    {
                        ...
                        this.formatter.push_format( format.indent, prefix, prefix + YAML.array_extra );
                        ++c_token.state.i;
         ...
         */
        var format_context = this.formatter.top_format();
        /*
        ...
        switch ( view.type.id )
        {
            ...
            case Encoding.type.object.id:
                ...
                for ( let i = 0 ; i < view.seq.length ; ++i )
                {
                    ...
                    this.sink( k + ":" );
                    this.formatter.push_format( format_context.indent + 1, " ", recurring_extra );
            ...
            case Encoding.type.array.id:
                ...
                {
                    token.state = { i: 0 };
                    this.formatter.push_format( format_context.indent + 1, "", YAML.array_extra );
            ...
            case Encoding.type.array_stream.id:
            {
                ...
                {
                    throw new Error( "Should only see 'array_stream' as input token once" );
                }
                break;
            }
            default:
