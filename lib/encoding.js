...
YAML_stream.prototype._machine = function()
{
    ...
    while ( this._has_token() || this._has_input() )
    {
         ...
         * us as a generator, we'll have an input token after a yield.
         */
        // Defense
            ...
            switch ( c_token.view.type.id )
            {
                ...
                case Encoding.type.array_stream.id:
                    ...
                    if ( 'mark' in la_token.content )
                    {
                        switch ( la_token.view.type.id )
                        {
                            case Encoding.type.start.id:
                                 ...
                                 * new list. In that case, it means we need a new node on the top of the stack.
                                 */
                                ...
                                if ( x && x.type.stream )
                                {
                                     ...
                                     * done so. If we have not already consumed then the present start token belongs to
                                     * a new, nested sequence.
                                     */
                                    need_lookahead = true;
                                }
                                else
                    ...
                    else
                    {
                        ...
                        if ( c_token.state.i == 0 )
                        {
                            this.sink( format.aggregate_extra + YAML.array_mark );
                        }
                        else
                        {
                            this.sink( tab( format.indent ) + YAML.array_mark );
                        }
                        ++c_token.state.i;
        ...
        switch ( view.type.id )
        {
            ...
            case Encoding.type.array.id:
                ...
                {
                    token.state = { i: 0 };
                    this._push_format( format_context.indent + 1, "", YAML.array_extra );
            ...
            case Encoding.type.start.id:
                throw new Error( "Mark 'start' should not appear as a value" );
                break;
            case Encoding.type.stop.id:
                throw new Error( "Mark 'stop' should not appear as a value" );
                break;
            default:
