...
YAML_stream.prototype.write = function( value, view )
{
    ...
    /*
     * Initialize the input stack with a single token, the one that will be expanded. Start the generator and bring it
     * to its first yield statement. It will stop if there are no stream inputs within the token, so we check for that.
     */
    this.push_token( view, {value: value} );
    this.push_format( 0, "", "" );
    var g = this._machine();
    this.generator = g;
    try
...
YAML_stream.prototype._machine = function()
{
    while ( this.has_token() )
    {
        ...
        else
        {
             ...
             */
            value = undefined;
            log( "Received mark = " + JSON.stringify( received_content.mark )
                + "\nold view = " + JSON.stringify( view ) );
            view = { type: received_content.mark };
            /*
        ...
        switch ( view.type.id )
        {
            ...
            case Encoding.type.array.id:
                ...
                {
                    token.state = { i: 0 };
                    this.push_format( format_context.indent, "", YAML.array_extra );
                }
            ...
            case Encoding.type.array_stream.id:
            {
                ...
                {
                    token.state = { i: 0 };
                    log( "array_stream, initial pass. top = " + JSON.stringify( this.top_token() ) );
                    let prefix = YAML.array_mark
                        + ( ( token.state.i == 0 ) ? format_context.aggregate_extra : tab( format_context.indent )
                        );
                    this.push_format( format_context.indent, prefix, prefix + YAML.array_extra );
                }
                /*
                 ...
                 * to precede.
                 */
                this.push_token( view.element_view );
                ++token.state.i;
                /*
                ...
                {
                    /*
                     * If this is the first time we see the array type, we need to push another frame onto the stack
                     * to ensure that the user calls sequence_start() and that we receive a mark.
                     */
                    this.push_token( { type: Encoding.type.start } );

                    let t = this.top_token();
                    log( "array_stream. 3. top of stack = " + JSON.stringify( this.top_token() ) );
                }
                break;
            }
            case Encoding.type.start.id:
                if ( token.view.type.id != Encoding.type.start.id )
                {
 ...
 * @param {*} token
 */
YAML_stream.prototype.push_token = function( view, token )
{
    var x = {};
    x.view = view;
...
YAML_stream.prototype.top_token = function()
{
    return this.stack[ this.stack.length - 1 ];
};

YAML_stream.prototype.pop_token = function()
{
...
YAML_stream.prototype.has_token = function()
{
