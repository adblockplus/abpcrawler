...
YAML_Format.prototype.array_begin = function()
{
    ...
    let prefix = parent.aggregate_first + YAML.array_mark;
    this.push_format( parent.indent + 1, prefix, prefix + YAML.array_extra );
...
YAML_Format.prototype.array_before_element = function()
{
    ...
    if ( format.counter > 0 )
    {
        let prefix = tab( parent.indent ) + YAML.array_mark;
        this.pop_format();
...
var YAML_stream = function( sink )
{
    ...
    this.formatter = new YAML_Format( sink );
};
 ...
 * @return {boolean}
 */
YAML_stream.prototype.may_write = function()
{
                    ...
                    if ( is_mark( la_token ) )
                    {
                        switch ( la_token.view.type.id )
                        {
                            case Encoding.type.start.id:
                                ...
                                {
                                    /*
                                     * Note that this is a special case where a token appears on the traversal stack
                                     * without first passing through the input queue.
                                     */
                                    this._push_token( { view: x } );
                                    /*
                                     * Important: We do not consume the input token here. If we have not already
                                     * consumed it, then the present start token belongs to a new, nested sequence. We
                                     * leave it in the input queue to be consumed at the next iteration, when the
                                     * token we just pushed will be on the top of the traversal stack.
                                     */
                                }
                                else
                                {
