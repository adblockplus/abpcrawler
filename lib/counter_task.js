...
var log = new Logger.make_log( "counter_task.js" );
log( "Loading" );
if ( !exports )
{
    log( "'exports' is not present" )
}
else
{
    log( "'exports' is present" )
}
*/
...
Counting_Task.prototype.generator = function( pause, resume )
{
     ...
     */
    var thread_manager = Cc["@mozilla.org/thread-manager;1"].createInstance( Ci.nsIThreadManager );
    ...
    for ( j = 0 ; j < this.count ; ++j )
    {
        if ( !pending )
        {
            thread_manager.currentThread.dispatch( { run: land }, Ci.nsIEventTarget.DISPATCH_NORMAL );
            pause();
            pending = true;
        }
        /*
