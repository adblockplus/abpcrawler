...
/**
 * @fileOverview The purpose of this module is to isolate the parameters needed to run a crawl from the means by which
 * those parameters are collected by top-level interface code. There are two basic forms of interface: interactive and
...
Application_Session.prototype.run = function( finally_f, catcher )
{
  this.finisher = finally_f;
  ...
  this.current_crawl = new Long_Task( this.current_crawler, this.instructions.size * 3 );
...
{
  if ( this.catcher ) this.catcher( ex );
  this._run_finally();
};
