  ...
  try
  {
    ...
    for ( instruction of gen )
    {
      ...
      if ( this.tabbed_browser.available() )
      {
         ...
         */
        tab = this.tabbed_browser.make_tab( instruction.target, this.leave_open, this._load_finisher.bind( this ), null );
        tab.instruction = instruction;
        instruction.begin();
        let join = new Action.Join_Timeout( tab, this.time_limit, this._land.bind( this ), this._join_catcher.bind( this ) );
        join.go( tab, resume );
        /*
...
Crawler.prototype._land = function()
{
   ...
   */
  Action.dispatch( Function.prototype.apply.bind( this._join_finisher, this, arguments ) );
};

Crawler.prototype._join_catcher = function( e, tab )
{
  tab.crawler_exception = e;
};

/**
 *
 */
Crawler.prototype._join_finisher = function( tab, resume )
{
  this._load_end_action( tab, resume );
};

/**
 *
 */
Crawler.prototype._load_finisher = function( tab )
{
  var s = "_load_finisher. ";
  s += "\n\ttab is " + (tab ? "not " : "") + "null";
  if ( tab )
  {
  ...
  Cu.reportError( s );
  tab.crawler_has_seen_load_finish = true;
};
...
Crawler.prototype._load_end_action = function( tab, resume )
{
   ...
   * then the tab action completed. In all cases, the tab will be complete afterwards.
   */
  if ( "crawler_exception" in tab )
  {
    tab.stop();
  }

  // Verify that the tab
  if ( !("crawler_has_seen_load_finish" in tab) || !tab.crawler_has_seen_load_finish )
  {
    Cu.reportError(
      "tab has not completed its load action when it was supposed to have."
        + "\n\ttab.target = " + tab.target
    );
  }
    ...
    if ( tab.completed_well )
    {
      switch ( tab._argv[1] )
      {
        ...
        case Browser_Tab.Completion_State.No_Success:
          instruction.abort( "unsuccessful load. nsresult = " + tab._argv[2] );
          break;
