...
Crawler.prototype.generator = function( pause, resume )
{
     ...
     * @param success
     */
    var land = function( tab, resume, success )
    {
        Cu.reportError( "Crawler / load end " + timestamp() );
    ...
    try
    {
        ...
        for ( instruction of this.instructions )
        {
            ...
            if ( this.tabbed_browser.available() )
            {
                ...
                Cu.reportError( "Crawler / load begin " + timestamp() );

                tab.load( instruction.target ).go( land.bind( this, tab, resume ), null );
            }
            if ( !this.tabbed_browser.available() )
            {
                pause();
...
/**
 * DO NOT USE YET. The variable 'loading' is still required For this function to work.
 *
 * @param tab
 * @param {Function} resume
 * @param success
 */
Crawler.prototype.land = function( tab, resume, success )
{
    Cu.reportError( "Crawler / load end " + timestamp() );
...
Crawler.prototype.node_action = function( original_f, wnd, node, contentType, location, collapse )
{
    ...
    try
    {
        ...
        {
            Cu.reportError(
                "Crawler.node_action: Browser not found in internal map. " + timestamp()
        ...
        log( "Observation: {content_type:" + contentType + ", n_entries:" + entries.length + ", location:" + url_location + "}" );
