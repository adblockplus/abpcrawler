...
Crawler.prototype.generator = function( pause, resume )
{
    ...
    finally
    {
        ...
        if ( tab )
        {
            // Assert constructor for 'tab' did not throw an error.
            tab.close();
        }
    }
};
...
{
    /*
     * This check forestalls a race condition where a request completes and schedules a progress event while we are
     * closing the object.
     */
    if ( this.state >= Browser_Tab.STATE.ERROR )
        return;
