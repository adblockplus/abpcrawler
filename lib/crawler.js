    ...
    try
    {
        ...
        for ( instruction of this.instructions )
        {
            ...
            if ( this.tabbed_browser.available() )
            {
                ...
                instruction.start();
                this.display.log( "-" );
                this.display.log( "    time-start: " + instruction.time_start );
                this.display.log( instruction.to_YAML_value( 1 ) );
                /**
...
Crawler.prototype._load_end_action = function( tab, resume, success )
{
    ...
    this.display.log( "    time-finish: " + tab.instruction.time_finish );
    tab.close();
    resume();
};
...
Crawler.prototype.node_action = function( original_f, wnd, node, contentType, location, collapse )
{
    ...
    try
    {
        ...
        var instruction = this.locate_instruction( wnd );
        ...
        try
        {
            ...
            if ( !Observation )
            {
                Cu.reportError( "Observation is not present" );
            }
            var observation = new Observation( !result, contentType, url_location, entries );

            this.display.log( "        -" );
            this.display.log( observation.to_YAML_value( 3 ) );
