  ...
  try
  {
    ...
    for ( instruction of gen )
    {
      ...
      if ( this.tabbed_browser.available() )
      {
         ...
         */
        tab = this.tabbed_browser.make_tab( instruction.target, this.leave_open, null, null );
        tab.instruction = instruction;
        instruction.begin();
        let join = new Action.Join_Timeout( tab, this.time_limit, this._land.bind( this ), this._load_catcher.bind( this ) );
        join.go( tab, resume );
        /*
...
Crawler.prototype._land = function()
{
   ...
   */
  Action.dispatch( Function.prototype.apply.bind( this._load_end_action, this, arguments ) );
};

Crawler.prototype._load_catcher = function( e, tab )
{
    ...
    if ( tab.completed_well )
    {
      switch ( tab._argv[0] )
      {
        ...
        case Browser_Tab.Completion_State.No_Success:
          instruction.abort( "unsuccessful load. nsresult = " + tab._argv[1] );
          break;
