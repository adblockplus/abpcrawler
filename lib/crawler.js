 ...
 *      Either "JSON" or "YAML".
 */
var Crawler = function( instructions, storage, display, window, leave_open, encoding )
{
    /**
...
Crawler.prototype.__encoding__ = Encoding.as_object( [
    // prelude
    Encoding.immediate_fields( ["time_start", "instructions", "storage"] ),
    // observation
...
Crawler.prototype.generator = function( pause, resume )
{
    ...
    try
    {
        ...
        var formatter = new Encoding[ this.encoding_type ]( this.storage.writer() )
        this.encoder = new Encoding.Format_stream( formatter );
        ...
        for ( instruction of gen )
        {
            ...
            if ( this.tabbed_browser.available() )
            {
                ...
                instruction.begin( this.encoder );
                /*
...
{
    tab.instruction.end( this.encoder, success );
    tab.close();
    resume();
};
