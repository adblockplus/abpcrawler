...
function loadSites(backendUrl, parallelTabs, window, sites)
{
  while (currentTabs < parallelTabs && sites.length)
  {
    ...
    loadSite(site, window, function()
    {
      ...
      {
        Client.sendCrawlerDataFile(backendUrl, window, Storage.dataFile,
                                   Storage.destroy);
      }
      else
        loadSites(backendUrl, parallelTabs, window, sites);
    });
  }
}
...
Crawler.crawl = function(backendUrl)
{
  ...
  // TODO: Don't reset this before all files are loaded
