...
{
    Cu.reportError( "Tabbed_Browser.close" );
    if ( this.listener )
    {
    ...
    for ( let pair of this.map_browser_to_child )
    {
        let [ key, value ] = pair;
        value.child.close();
...
Tabbed_Browser.prototype._progress = function( browser, controller, browse_request, state, stop_status )
{
    ...
    {
        s += "Tabbed_Browser: _progress() did not find 'browser' object in its internal map.\n";
    }
